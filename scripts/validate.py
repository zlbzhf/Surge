...
def check_bsbsb_module() -> list[Issue]:
    ...
    module_checks = [
        ...
        ("bilibili-bsbsb.airborne.js?v=20260523-chronos-opt-in-1", "module should cache-bust the airborne script URL after adding opt-in Chronos auto-seek"),
        ("自动跳:#", "Chronos auto-seek must stay disabled by default until field validation passes"),
        ("DmSegMobile", "module should hook the Bilibili danmaku segment endpoint"),
        ("bilibili.bsbsb.chronos", "module should include a separately gated optional Chronos response hook"),
        ("type=http-response", "optional Chronos hook should run as an HTTP response script only when 自动跳 enables it"),
        ("ViewProgress", "optional Chronos hook should only target Bilibili ViewProgress responses"),
        ('"sponsorBlock":"{{{自动跳}}}"', "optional Chronos hook should be gated by the 自动跳 parameter, not the main helper switch"),
        ('"uiObservation":"0"', "optional Chronos hook must keep UI observation disabled in the stable module"),
        (BSBSB_SCRIPT_URL, "module should use the same-repo script for danmaku injection and optional Chronos patching"),
    ...
    doc_checks = [
        ...
        ("响应重写默认关闭", "docs should explain that response rewriting is disabled in the stable default module"),
        ("自动跳 = #", "docs should document that Chronos auto-seek is disabled by default"),
