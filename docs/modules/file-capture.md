...
归档服务会按 SHA256 去重；如果同一文件先被放进 `未关联产品/文件`，后续 SOP/产品页带来更明确的产品名和资料类型，会移动到更准确目录而不是复制一份。扩展名以 magic bytes / Content-Type 校正，不只信 URL 后缀。无明确标签的大图会按尺寸粗分：超长图优先归为 `一图`，横向大图优先归为 `宣传彩页`，其他进入 `待确认图片资料`；小图标类素材会按大小和尺寸（如 500×500 内）识别，新捕获直接跳过，历史整理时隔离到 `忽略小图标`。PDF 如安装了 `pdftotext`，服务端会读取前几页标题/正文信号，把误继承的“产品条款”纠正为 `费率表`、`产品说明书`、`营运规则` 等。
