...
- **开头汇总弹幕**默认开启：优先在第一个弹幕分段中插入多条小字号、分行式空降卡片提示，不再固定写死在视频第 3 秒，而是读取 `DmSegMobile` 请求里的实际播放窗口 `ps` / `pe`，按“当前进入播放位置 + 汇总弹幕毫秒”（默认 3000ms）计算出现时间；如果请求里没有播放窗口，才回退到弹幕分段开头约 3000ms。用来提示本视频包含哪些类型、各类型数量/时长以及是否可自动跳过或手动空降。
