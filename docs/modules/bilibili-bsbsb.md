...
- **开头汇总弹幕**默认开启：优先在第一个弹幕分段中插入一条小字号、短文案的空降卡片式汇总提示，默认约 3000ms 出现，用来提示本视频将自动跳过几段、约多少时长以及是否有高能点。
