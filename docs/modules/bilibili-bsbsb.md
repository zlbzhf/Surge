...
- **开头汇总弹幕**：默认开启。脚本会在第一个 `DmSegMobile` 弹幕分段中追加多条小字号、分行式空降卡片提示，默认约 3000ms 出现，例如：`空降助手提示`、`本视频含 1 段广告，1分14秒，将为您自动跳过`、`本视频含 2 个高能，可手动空降`。每个 skip 类型单独生成一条，`poi_highlight` 单独生成一条，达到类似换行的阅读效果。如果视频开头本身有会被自动跳过的片段，脚本会把汇总弹幕顺延到该片段目标后约 1 秒，避免 800ms 这类过早时间点在请求完成或自动跳转后被错过。如果首个 `DmSegMobile` 请求因 bsbsb 查询超时而错过，脚本也会在包含第一个有效空降片段的弹幕分段里补一组汇总提示，并把时间点放在该分段开头附近。这些提示使用 `airborne:<自身时间点>` 走与已验证空降提示相同的可见路径，但内容不会使用精确的 `空指部已就位`，因此不会触发 Chronos 自动跳逻辑；误点也只会跳到它自身附近。
